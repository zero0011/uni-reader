<template>
  <div :class="['chapter-page', { nightMode: isNightMode }]">
    ...
    <div class="turnPage" v-show="chapterDetailsConver">
      ...
      <div class="button next" @click="gotoTargeChapter(currentPageIndex + 1)"
        v-if="currentPageIndex < chaptersSectionCount - 1">下一章</div>
    </div>
    <!-- 底部菜单 -->
    <div :class="['chapter-footbar', { show: showFooterBar }]">
      ...
      <div class="setting-panel" v-show="showFooterBar && showSettingPanel">
        ...
        <div class="lightness setting-panel-normal">
          ...
          <slider min="0" max="1" :value="lightness" step="0.05" color="#f5f5f5" activeColor="#4393e2" block-size="14"
        ...
        <div class="font-size setting-panel-normal">
          ...
          <slider min="12" max="30" :value="chapterFontSize" step="1" color="#f5f5f5" activeColor="#4393e2"
            block-size="14" @change="changeFontSize" />
        </div>
      </div>
    </div>
    <!-- 目录 -->
    <section :class="['chapter-picker', { showDirectory }]" @click="toggleCategoryList">
      <header @click.stop>
        ...
        <picker :range="chapterSectionArray" @change="chapterSectionArrayChange" :value="currentChapterSectionIndex">
          <span class="picker">{{ chapterSectionArray[currentChapterSectionIndex] }} <i
      ...
      <ul @click.stop>
        <li :key="item.id"
          ...
          v-for="(item, index) in currentChapterSection" @click="gotoTargeChapterFromItem(item, index)">{{ item.title }}
        </li>
      </ul>
      <button>关闭</button>
    </section>
...
export default {
  ...
  methods: {
    ...
    gotoSourcePage(bookId) {
      ...
      if (!this.bookInBookCase) {
        ...
        wx.navigateTo({
          url: `/pages/source?bookId=${bookId}`
        });
      }
    },
    // 改变亮度
  ...
  onShareAppMessage() {
    ...
    return {
      ...
      path: `/pages/chapter?from=share&bookId=${_id}&sourceId=${_currentSource._id}&chapterIndex=${this.currentPageIndex}`,
