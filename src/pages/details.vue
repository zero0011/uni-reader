<template>
  <div class="detail-page">
    ...
    <div class="detail-likes" v-if="booksLike.length > 0">
      <h5>猜你喜欢</h5>
      <scroll-view scroll-y style="width: 100%; white-space: normal;">
...
export default {
  ...
  methods: {
    ...
    gotoChapterPage(bookId = '') {
      wx.navigateTo({
        url: `/pages/chapter?bookId=${bookId}`
      });
    },
    // 跳转详情页,同一个页面不能跳转？
    // 用的同一实例？
    ...
    gotoDetailsPage(bookId) {
      if (bookId) {
        wx.navigateTo({
          url: `/pages/details?bookId=${bookId}`
        });
      }
    },
    // 获取详情及推荐
  ...
  onShareAppMessage() {
    return {
      ...
      path: `/pages/details?bookId=${this.bookDetails._id}`,
